...
import { getFavAsync } from "@/store/auth";
...
export const RentCard = ({ rent }: Props) => {
  ...
  const mainImage = !imageUrls[0].includes("undefinedpublic")
    ? imageUrls[0]
    : noImage;
  ...
  return (
    <div className="w-full bg-white rounded-[10px] p-4 lg:p-6">
      ...
      >
        ...
        />
        <div className="bg-[linear-gradient(180deg,rgba(255,255,255,0.00)0%,#FFF_100%)] w-full h-[28px] absolute bottom-0" />
      </Link>
